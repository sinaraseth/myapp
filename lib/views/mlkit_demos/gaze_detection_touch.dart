...
import '../../controllers/gaze_detection_touch_controller.dart';
import '../../models/gaze_result.dart';

class GazeDetectionTouch extends StatefulWidget {
  const GazeDetectionTouch({super.key});

  @override
  State<GazeDetectionTouch> createState() => _GazeDetectionTouchState();
}

class _GazeDetectionTouchState extends State<GazeDetectionTouch>
    with SingleTickerProviderStateMixin {
  final GazeDetectionTouchController _controller =
      GazeDetectionTouchController();
  ...
  static const double _edgePadding = 0.0; // Reduced padding to push dots further to edges

  /// Returns the centre offset of the dot for [target] within [size].
  Offset _dotPosition(String target, Size size) {
    ...
    switch (target) {
      ...
      case 'UP':
        return Offset(cx, _edgePadding + _dotRadius + 50); // clear AppBar
      case 'DOWN':
        return Offset(cx, size.height - _edgePadding - _dotRadius - 70);
      case 'CENTER':
      default:
        return Offset(cx, cy);
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Gaze Detection (Touch)'),
      ...
      child: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.touch_app, size: 80, color: Colors.cyanAccent),
              const SizedBox(height: 24),
              const Text(
                'Gaze Detection by Touch',
                style: TextStyle(
                  color: Colors.white,
                  fontSize: 24,
  ...
  Widget _buildResultScreen() {
    final allPassed = _controller.tapResults.entries.every((e) => e.value?.direction == e.key);
    return Container(
      color: Colors.black,
      child: SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 16),
              Text(
                allPassed ? 'You are Verified! 🎉' : 'Verification Incomplete',
                style: TextStyle(
                  color: allPassed ? Colors.greenAccent : Colors.orangeAccent,
                  fontSize: 26,
                  fontWeight: FontWeight.bold,
              ...
              ...GazeDetectionTouchController.sequence.map((dir) {
